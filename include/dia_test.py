...
if __name__ == '__main__':
    ...
    intrinsics = depth_image_annotator.Intrinsics(ppx=644.489, ppy=358.173, fx=922.249, fy=922.439, left=0.0, right=1280.0, bottom=720.0, top=0.0, zNear=0.1, zFar=3.0)
    bbox = depth_image_annotator.Box(x=563, y=338, width=164)
    ...
    loaded_depth_image = cv2.imread('/home/eric/Dev/DepthImageAnnotator/include/annoying_case.exr', cv2.IMREAD_UNCHANGED)
    loaded_depth_image = loaded_depth_image.flatten()
    params = dia.FindSolution(depth_data=loaded_depth_image, w=164, h=164, is_left=False, bbox=bbox, intrinsics=intrinsics, iterations=60)
    ...
    depth_array_1 = dia.WriteImage(currentdt=256*256, w=256, h=256, params=params.FirstBest, is_left=False, intrinsics=intrinsics)
    depth_array_1 = depth_array_1.reshape(256, 256)
    depth_array_2 = dia.WriteImage(currentdt=256*256, w=256, h=256, params=params.SecondBest, is_left=False, intrinsics=intrinsics)
    depth_array_2 = depth_array_2.reshape(256, 256)
    depth_array_3 = dia.WriteImage(currentdt=256*256, w=256, h=256, params=params.ThirdBest, is_left=False, intrinsics=intrinsics)
    depth_array_3 = depth_array_3.reshape(256, 256)
    depth_array_4 = dia.WriteImage(currentdt=256*256, w=256, h=256, params=params.FourthBest, is_left=False, intrinsics=intrinsics)
    depth_array_4 = depth_array_4.reshape(256, 256)
    depth_array_5 = dia.WriteImage(currentdt=256*256, w=256, h=256, params=params.FifthBest, is_left=False, intrinsics=intrinsics)
